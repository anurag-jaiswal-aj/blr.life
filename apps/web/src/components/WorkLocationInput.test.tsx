...
describe("WorkLocationInput", () => {
  ...
  describe("Search Functionality", () => {
    it("renders search input with accessible semantic labels", () => {
      ...
      expect(
        screen.getByLabelText(/Search for a work location/i),
      ).toBeInTheDocument();
    ...
    it("typing alone DOES NOT call geocoding", async () => {
      const searchSpy = vi
        .spyOn(geocodingModule, "searchPlaces")
        .mockResolvedValue([]);
      render(<WorkLocationInput state={defaultState} updateState={vi.fn()} />);
    ...
    it("displays search results and allows selection via explicit Search button click", async () => {
      ...
      render(
        <WorkLocationInput state={defaultState} updateState={updateSpy} />,
      );
      ...
      expect(updateSpy).toHaveBeenCalledWith(
        { lat: 12.9352, lng: 77.6245, loc: "Koramangala, Bangalore" },
        { history: "push" },
      );
    });
    ...
    it("displays search results and allows selection via Enter key", async () => {
      ...
      expect(updateSpy).toHaveBeenCalledWith(
        { lat: 12.9716, lng: 77.5946, loc: "Bangalore, India" },
        { history: "push" },
      );
    });

    it("shows error state when search fails", async () => {
      vi.spyOn(geocodingModule, "searchPlaces").mockRejectedValue(
    ...
    it("clears selected location and resets state on clear button click", async () => {
      ...
      rerender(
        <WorkLocationInput
          state={{ ...defaultState, lat: 12.9716, lng: 77.5946 }}
      ...
      const clearBtn = screen.getByRole("button", {
        name: /Clear selected location/i,
      });
