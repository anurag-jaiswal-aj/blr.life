...
export function RecommendationWorkspace() {
  ...
  const [hoveredLocalityId, setHoveredLocalityId] = useState<number | null>(
    null,
  );
  ...
  const handleToggleSave = useCallback(
    (localityId: number) => {
      updateState(
        {
          saved_ids: state.saved_ids.includes(localityId)
            ? state.saved_ids.filter((id) => id !== localityId)
            : [...state.saved_ids, localityId],
        },
        { history: "replace" },
      );
    },
  ...
  const displayRecommendations = React.useMemo(() => {
    ...
    if (activeView === "saved") {
      return data.recommendations.filter((r) =>
        state.saved_ids.includes(r.locality_id),
      );
    }
    return data.recommendations;
  }, [data, activeView, state.saved_ids]);
  ...
  useEffect(() => {
    ...
    const stillExists = displayRecommendations.some(
      (r) => r.locality_id === selectedLocalityId,
    );
    if (!stillExists) {
      setSelectedLocalityId(displayRecommendations[0]?.locality_id ?? null);
    }
  }, [displayRecommendations, selectedLocalityId, data?.recommendations]);
        ...
        <div className="animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="flex flex-wrap gap-1.5">
            {[
              ...
              {
                label: "Dining",
                key: "w_restaurant" as const,
                value: state.w_restaurant,
              },
              { label: "Parks", key: "w_park" as const, value: state.w_park },
              {
                label: "Healthcare",
                key: "w_healthcare" as const,
                value: state.w_healthcare,
              },
              {
                label: "Nightlife",
                key: "w_nightlife" as const,
                value: state.w_nightlife,
              },
            ].map(({ label, key, value }) => {
              const level =
                value >= 1.0 ? "High" : value >= 0.5 ? "Low" : "Off";
  ...
  return (
    <div className="flex flex-col bg-surface-app text-text-primary font-sans h-[100dvh] overflow-hidden">
      {/* HEADER */}
      <header
        className={`flex items-center justify-between py-2.5 bg-surface-primary border-b border-border-default shrink-0 z-50 ${hasLocation ? "px-4 md:px-6" : "px-5 lg:px-8"}`}
      >
        <div className="flex items-center gap-4">
          <div
            className="text-[14px] font-bold text-brand-primary tracking-[0.15em] uppercase cursor-pointer"
            onClick={() => updateState({ lat: null, lng: null })}
          >
            blr.life
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col lg:flex-row w-full relative overflow-hidden">
        ...
        <section
          className={`hidden lg:flex shrink-0 h-full flex-col border-r border-border-default bg-surface-app z-10 overflow-hidden transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${
            hasLocation
              ? "w-[320px] xl:w-[340px]"
              : "w-[40%] max-w-[560px] min-w-[400px]"
          }`}
        >
          {/* ALWAYS show setup controls */}
          <div
            className={`shrink-0 border-border-default bg-surface-primary ${hasLocation ? "border-b" : "flex-1 flex flex-col justify-center px-4 pb-[15vh]"}`}
          >
            {renderSetupControls()}
          </div>
              ...
              <div className="flex-1 overflow-y-auto bg-surface-app">
                {!mounted || isDesktop ? (
                  <RecommendationList
                    ...
                    savedLocalityIds={state.saved_ids}
          ...
          <section className="hidden lg:flex w-[360px] xl:w-[400px] shrink-0 h-full flex-col border-r border-border-default bg-surface-primary z-10 relative animate-in slide-in-from-left-4 fade-in duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]">
            {selectedRecommendation ? (
              <NeighbourhoodDetail
                ...
                isSaved={state.saved_ids.includes(
              ...
              <div className="flex-1 flex items-center justify-center p-6">
                <p className="text-[13px] text-text-muted text-center max-w-[250px]">
                  {loading
                    ...
                    : "Select a neighbourhood to see details"}
                </p>
              </div>
            )}
          </section>
          ...
          {mounted &&
            ...
            (hasLocation ? (
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-surface-primary rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.12)] z-40">
                ...
                <div className="px-2 pb-6">{renderSetupControls()}</div>
              </div>
            ))}
        </div>
      </main>
