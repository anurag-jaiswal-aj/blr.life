...
import { RecommendationResponse, RecommendationResult } from "../lib/api";
...
export function RecommendationList({
    ...
    return (
      <div className="flex flex-col divide-y divide-border-subtle">
        {isColdStarting && (
          <div className="p-4 bg-brand-light/30 text-center animate-pulse-slow">
            <p className="text-sm font-medium text-brand-primary">
              Starting the recommendation service &mdash; this can take a little
        ...
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="animate-pulse bg-surface-primary p-4 md:p-5 flex flex-col justify-between"
          >
  ...
  const actualList = displayRecommendations || data.recommendations;
  ...
  if (actualList.length === 0) {
      ...
      return (
        <div className="p-6 bg-surface-primary text-center py-10">
          ...
          <p className="text-xs text-text-secondary mt-1">
            Tap the heart icon on any neighbourhood to save it.
          </p>
        </div>
      );
    }
    return (
      <div className="p-6 bg-surface-primary text-center py-10">
        ...
        <p className="text-xs text-text-secondary mt-1">
          Adjust your workplace or maximum distance.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col divide-y divide-border-subtle">
      {actualList.map((rec) => (
        <ResultCard
          ...
          isSaved={savedLocalityIds.includes(rec.locality_id)}
      ...
      <div className="p-4 bg-surface-secondary text-[11px] text-text-secondary text-center uppercase tracking-wider font-bold">
        Results powered by blr.life deterministic engine.
        <br />
        <span className="opacity-75">
          v: {data.provenance.calc_versions_used.join(", ")}
        </span>
      </div>
    </div>
  );
