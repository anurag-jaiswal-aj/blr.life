...
async def test_recommend_affordability(
    ...
    async_db_session.add_all([r1, r2, r4])
    await async_db_session.commit()
    payload = {
        "work_location": {"lat": 12.9716, "lng": 77.5946},
    ...
    assert l3_rec["affordability"]["status"] == "unknown"
...
async def test_recommend_affordability_insufficient(
    ...
    assert "near-metro" in slugs
    l1_rec = next(r for r in recs if r["slug"] == "near-metro")
    assert l1_rec["affordability"]["status"] == "unknown"
    assert l1_rec["affordability"]["rent_min_inr"] is None
    assert l1_rec["affordability"]["rent_max_inr"] is None
    assert l1_rec["affordability"]["confidence"] is None
